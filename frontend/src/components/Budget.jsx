...
const Budget = ({ placeId }) => {
  ...
  useEffect(() => {
    const fetchItinerary = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/itinerary/${placeId}`, { credentials: 'include' });
        const data = await res.json();
        // adapt to backend itinerary format
        const days = (data.data?.itinerary || []).map((d, idx) => ({
          day: idx + 1,
          activities: (d.stops || []).flatMap(s => (s.stop?.activities || []).map(a => ({
            activity: a.activity?.name || 'Activity',
            expense: a.activity?.cost ?? '-'
          })))
        }));
        setItinerary(days);
      } catch (err) {
